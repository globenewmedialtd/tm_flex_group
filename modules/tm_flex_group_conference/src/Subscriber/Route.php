...
namespace Drupal\tm_flex_group_conference\Subscriber;
  ...
  protected function alterRoutes(RouteCollection $collection) {
    ...
    foreach ($join_routes as $name => $argument) {
      if ($route = $collection->get($name)) {
        ...
        $requirements = array_merge($current, ['_tm_conference_join_permission' => $argument]);
    ...
    \Drupal::moduleHandler()
      ->alter('tm_flex_group_conference_content_routes', $content_routes);

    foreach ($content_routes as $name) {
      if ($route = $collection->get($name)) {
        $current = $route->getRequirements();
        $requirements = array_merge($current, ['_tm_conference_content_visibility' => 'public']);
        $route->addRequirements($requirements);
      }
    }
  }
